"""
Verify data in bizpulse production database
This script checks the data loaded into the production database
"""
import asyncio
import os
...
async def verify_production_database():
    """Verify data in production database (reads DB_NAME from .env)"""
    ...
    logger.info(f"📊 Connecting to MongoDB PRODUCTION database: {DB_NAME}")
    ...
    if total_count == 0:
        logger.warning("⚠️  Database is empty! Please run sync_azure_data_production.py first.")
        return
    ...
    if count_2025 > 0:
        ...
        logger.warning("⚠️  No 2025 data found in production database")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(f"🔍 Verifying {db_name} PRODUCTION Database")
    print("=" * 60)
    print()
    asyncio.run(verify_production_database())
