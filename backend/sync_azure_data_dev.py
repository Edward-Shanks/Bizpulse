"""
Sync data from Azure Blob Storage to MongoDB Development Database
This script loads data from Biz-Pulse/yearly_data1.csv into bizpulseDev database
"""
import asyncio
import os
...
async def load_data_to_dev_db():
    """
    Load data from Azure Blob Storage (yearly_data1.csv) into bizpulseDev database
    """
    # MongoDB connection - using bizpulseDev database
    MONGO_URL = os.getenv('MONGO_URL')
    DB_NAME = os.getenv('DB_NAME', 'bizpulseDev')  # Default to bizpulseDev if not set
    ...
    # New file path: Biz-Pulse/yearly_data1.csv
    blob_path = os.getenv('AZURE_BLOB_PATH_DEV', 'Biz-Pulse/yearly_data1.csv')
    ...
    logger.info(f"\n🎉 Successfully loaded {total} records into {DB_NAME} database!")
    ...
    if count_2025 > 0:
        ...
        logger.info(f"   2025 Months: {[m['_id'] for m in month_stats]}")
    else:
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 Azure Data Sync to Development Database (bizpulseDev)")
    print("=" * 60)
    print("This script will:")
    print("  1. Connect to MongoDB (bizpulseDev database)")
    print("  2. Download yearly_data1.csv from Azure Blob Storage")
    print("  3. Load all data into the development database")
    print("=" * 60)
    print()
    
    asyncio.run(load_data_to_dev_db())
