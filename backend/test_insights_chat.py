"""
Test script for Insights Chat endpoint
"""
import requests
import json
import sys
...
def test_insights_chat():
    """Test Insights Chat endpoint"""
    print_status("\n" + "="*60, "BOLD")
    print_status("Testing Insights Chat Endpoint", "BOLD")
    ...
    # Step 2: Test Insights Chat
    print_status("\nStep 2: Testing Insights Chat...", "INFO")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print_status(f"✅ Insights Chat successful!", "SUCCESS")
            print_status(f"   Response length: {len(data.get('response', ''))} characters", "INFO")
            print_status(f"   Has pivot table: {bool(data.get('data', {}).get('pivot_table'))}", "INFO")
            pivot_table = data.get('data', {}).get('pivot_table', [])
            print_status(f"   Pivot table items: {len(pivot_table)}", "INFO")
            print_status(f"   Has follow-up questions: {bool(data.get('data', {}).get('follow_up_questions'))}", "INFO")
            print_status(f"\nResponse preview (first 300 chars):", "INFO")
            print(f"   {data.get('response', '')[:300]}...")
            if pivot_table:
                print_status(f"\nPivot table preview (first 3 items):", "INFO")
                for idx, item in enumerate(pivot_table[:3], 1):
                    print(f"   {idx}. {item}")
            return True
        else:
